...
const flagTypeConfig: Record<string, { color: string; icon: string; label: string }> = {
    gps_spoof: { color: Colors.danger, icon: 'location-outline', label: 'GPS Spoof' },
    rating_inflation: { color: Colors.warning, icon: 'star-outline', label: 'Rating Inflation' },
    impossible_route: { color: Colors.danger, icon: 'map-outline', label: 'Impossible Route' },
    ...
    new_account_abuse: { color: Colors.warning, icon: 'person-add-outline', label: 'New Account Abuse' },
};
    ...
    return (
        <ScrollView style={{ flex: 1, backgroundColor: Colors.background }} contentContainerStyle={{ paddingBottom: 100 }}>
            <View style={{ paddingTop: 60, paddingHorizontal: 20 }}>
                ...
                <Text style={{ fontSize: 13, color: Colors.textMuted, marginTop: 2 }}>{flags.filter((f) => f.status === 'pending').length} pending review</Text>
            </View>
                    ...
                    return (
                        <View key={flag._id} style={{ margin: 16, marginBottom: 8, backgroundColor: Colors.white, borderRadius: 12, padding: 16, borderLeftWidth: 4, borderLeftColor: config.color }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                ...
                                <View style={{ flex: 1 }} />
                                <View style={{ backgroundColor: flag.status === 'pending' ? Colors.background : Colors.border, paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 }}>
                                    <Text style={{ fontSize: 11, color: flag.status === 'pending' ? Colors.warning : Colors.textMuted }}>{flag.status}</Text>
                                </View>
                            </View>
                            <Text style={{ fontSize: 15, fontWeight: '600', color: Colors.text, marginTop: 8 }}>{flag.userName}</Text>
                            <Text style={{ fontSize: 13, color: Colors.textMuted, marginTop: 4 }}>{flag.evidenceSummary}</Text>
                            ...
                            {flag.status === 'pending' && (
                                <View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}>
                                    <Pressable onPress={() => handleAction(flag._id, 'dismissed')} style={{ flex: 1, paddingVertical: 8, borderRadius: 6, borderWidth: 1, borderColor: Colors.border, alignItems: 'center' }}>
                                        <Text style={{ fontSize: 13, color: Colors.textMuted }}>Dismiss</Text>
                                    </Pressable>
                                    <Pressable onPress={() => handleAction(flag._id, 'investigating')} style={{ flex: 1, paddingVertical: 8, borderRadius: 6, backgroundColor: Colors.background, alignItems: 'center' }}>
                                        <Text style={{ fontSize: 13, color: Colors.warning, fontWeight: '600' }}>Investigate</Text>
                                    </Pressable>
                                    <Pressable onPress={() => handleAction(flag._id, 'suspended')} style={{ flex: 1, paddingVertical: 8, borderRadius: 6, backgroundColor: Colors.white, borderColor: Colors.danger, borderWidth: 1, alignItems: 'center' }}>
                                        <Text style={{ fontSize: 13, color: Colors.danger, fontWeight: '600' }}>Suspend</Text>
                                    </Pressable>
                                </View>
                            )}
                        </View>
